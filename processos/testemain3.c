...
int main() {
    ...
    while (fgets(linha, sizeof(linha), processos) != NULL && i < tam) {
        ...
        int valoresLidos = sscanf(linha, "%lld,%49[^,],%49[^,],%49[^,],%49[^,],%d",
            ...
            /*if (sscanf(process[i].data_ajuizamento, "%d-%d-%d %d:%d:%d", 
                &process[i].ano, &process[i].mes, &process[i].dia, 
                &process[i].hora, &process[i].minuto, &process[i].segundo) == 6) {
            } else {
                printf("Erro ao processar data_ajuizamento: %s\n", process[i].data_ajuizamento);
            }*/
        ...
        if (quant_assunto[0] == '"') {
            ...
            valoresLidos = sscanf(linha, "%lld,%49[^,],%49[^,],%49[^,],\"%49[^\"]\",%d",
                &process[i].id, process[i].numero, process[i].data_ajuizamento,
                quant_classe, quant_assunto, &process[i].ano_eleicao);
